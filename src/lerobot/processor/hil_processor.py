    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        if is_intervention and teleop_action is not None:
            if isinstance(teleop_action, dict):
                # End-effector teleops expose deltas, while Piper leader exposes
                # ordered joint keys such as joint_1.pos ... gripper.pos.
            ...
            if teleop_action_tensor.shape != action.shape:
                raise ValueError(
                    "Teleop intervention action shape does not match policy action shape: "
                    f"teleop={tuple(teleop_action_tensor.shape)}, policy={tuple(action.shape)}"
                )
