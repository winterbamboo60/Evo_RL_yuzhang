...
def make_processors(
    ...
    """Create environment and action processors.创建环境处理器和动作处理器。

    Args:
        env: Robot environment instance. 机器人环境实例。
    ...
    # （必备）把环境原始 observation 转成 LeRobot policy 使用的标准 observation 字段，例如 observation.state、图像字段等。
    env_pipeline_steps = [VanillaObservationProcessorStep()]

    # 参考离线RL的做法，这里目前都是false
    ...
    # Add gripper penalty processor if gripper config exists and enabled 如果夹爪配置存在且已启用，则添加夹爪惩罚处理器
    # Only add if max_gripper_pos is explicitly configured (required for normalization) 仅当显式配置了 max_gripper_pos 时才添加（归一化所必需）。
    # 会添加 但是当前 gripper_penalty=0.0，所以即使处理夹爪 penalty，也不会实际扣 reward
    if (
    ...
    # 给 observation tensor 增加 batch 维度，让 policy 可以直接处理
    env_pipeline_steps.append(AddBatchDimensionProcessorStep())
    # 把 observation tensor 移到 cfg.policy.device，当前是 cuda。
...
def step_env_and_process_transition(
    ...
    # 环境 reward 加上 action processor 产生的 reward/penalty。
    # TODO 这里的Reward逻辑需要进一步确认
    reward = reward + processed_action_transition[TransitionKey.REWARD]
    terminated = terminated or processed_action_transition[TransitionKey.DONE]
    truncated = truncated or processed_action_transition[TransitionKey.TRUNCATED]
    # 保留 complementary data，例如 teleop_action、discrete_penalty。
