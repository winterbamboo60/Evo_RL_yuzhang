        ...
        for (src_chunk_idx, src_file_idx), episodes_in_file in tqdm(
            ...
            if all_in_file_set == episodes_to_keep_set:
                ...
                for old_idx in sorted_keep_episodes:
                    new_idx = episode_mapping[old_idx]
                    src_ep = src_dataset.meta.episodes[old_idx]
                    # Advance by the ACTUAL span of frames kept by `_keep_episodes_from_video_with_av`,
                    # which copies every frame in [from_ts, to_ts). Some source recordings have segments
                    # where (to_ts - from_ts) != length / fps (dropped frames / inter-episode gaps baked
                    # into the segment); using `length / fps` here would make the metadata timestamps drift
                    # away from the re-encoded video content (the drift accumulates over the file) and would
                    # disagree with the direct-copy branch above, desynchronizing cameras whose files are
                    # split at different episode boundaries.
