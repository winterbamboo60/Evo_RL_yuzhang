...
Timeout behaviour:
- When an episode ends by reaching the max recording time (timeout) without an explicit
  success/failure label, `--reset_on_timeout` controls what the arms do next:
    --reset_on_timeout=true  (default) -> drive the arms back to the initial pose (复位).
    --reset_on_timeout=false           -> hold the current pose, arms stay in place (保持原地不动).
"""
...
class _HumanInloopFailureResetController:
    def __init__(self, cfg: RecordConfig):
        ...
        self.failure_reset_pose: dict[str, float] | None = None  # 初始姿态
    ...
    def on_reset_episode(self, robot: Any, teleop: Any) -> None:
        """Called when the user presses the reset key (R) to abort and re-record the episode.

        Drives every arm back to the stored initial pose before the episode restarts, reusing the
        same slow-homing routine as the success/failure end-of-episode reset.
        """
        if self.failure_reset_pose is None:
            logging.warning("Reset key pressed but no initial reset pose is available; skipping homing.")
            return
        logging.info("Reset key pressed: homing arms to the initial pose before re-recording the episode.")
        _slow_reset_all_arms_to_pose(robot=robot, teleop=teleop, target_pose=self.failure_reset_pose)
...
def human_inloop_record(cfg: RecordConfig):
    ...
    if cfg.policy is not None:
        ...
        # 按下 R 键：放弃当前 episode，将机械臂慢速恢复到初始位置后，重新开始录制当前 episode。
        cfg.reset_episode_key = "r"
        cfg._on_record_reset_episode = failure_reset_controller.on_reset_episode
