...
class ActorPipelineConfig(TrainRLServerPipelineConfig):
    """Single configuration for local VLA collection or online Actor collection."""

    actor_mode: str = "vla_only"
...
def _normalize_actor_config(cfg: ActorPipelineConfig) -> None:
    if cfg.actor_mode not in {"vla_only", "online_actor"}:
        raise ValueError("actor_mode must be vla_only or online_actor")
    if cfg.save_format not in {"lerobot", "transition"}:
        raise ValueError("save_format must be lerobot or transition")
    if cfg.actor_mode == "online_actor" and cfg.save_format != "transition":
    ...
    if cfg.actor_mode == "online_actor":
        if cfg.dataset is None:
            raise ValueError("online_actor mode requires dataset metadata")
        if cfg.policy is None or not cfg.policy.pretrained_path:
            raise ValueError("online_actor mode requires policy.pretrained_path")
        if _find_actor_checkpoint(cfg.actor_checkpoint_path) is None:
            raise ValueError("online_actor mode requires a valid actor_checkpoint_path")
    cfg.validate()


def _build_actor_control(cfg: ActorPipelineConfig) -> ActorControl:
    needs_metadata = cfg.actor_mode == "online_actor" or cfg.task_hotkeys_path is not None
    dataset_meta = None
    if needs_metadata:
        if cfg.dataset is None:
            raise ValueError("dataset metadata is required for online_actor or task hotkeys")
    ...
    if task_hotkeys is not None:
        ...
        if unknown:
            raise ValueError(f"Task hotkeys missing from meta/tasks.parquet: {unknown}")
...
class ActorKeyboardController(KeyboardController):
    def __init__(self, control: ActorControl):
        ...
        self._actor2_events: ThreadQueue[str] = ThreadQueue()
    ...
    def push(self, key: str) -> None:
        ...
        if normalized == "b" or normalized in tasks:
    ...
    def poll(self, state):
        ...
        while True:
            ...
            if key == "b":
                ...
                logging.info(
                    "[ACTOR] Action mode switched to %s.",
                    "online Actor" if self.control.use_actor else "VLA-only",
                )
                continue
            ...
            logging.info("[ACTOR] Task key %s selected %r; discarding current episode.", key, task)
        return state
...
class ActorEpisodeWriter:
    ...
    def _ensure_lerobot_dataset(self, transition: Transition) -> LeRobotDataset:
        ...
        if not features:
            for key, tensor in transition["state"].items():
                ...
                names = (
                    self.action_names
                    if key == OBS_STATE and value.numel() == len(self.action_names)
                    else None
                )
            ...
            frame.update(
                {
                    ...
                    "complementary_info.is_intervention": np.array([is_intervention], dtype=np.float32),
            ...
            frames.append(
                {
                    ...
                    "state": _as_jsonable(
                        {k: v for k, v in state.items() if "image" not in str(k) and "pixels" not in str(k)}
                    ),
    ...
    def _write_viewer(
        self, episode_dir: Path, metadata: dict[str, Any], frames: list[dict[str, Any]]
    ) -> None:
...
class OnlineActorRuntime(ActorVLARuntime):
    """Run the trained online Actor head on top of frozen VLA/RLT features."""
    ...
    def __init__(self, control: ActorControl):
        ...
        if self.policy is None or self.policy_cfg is None or not self.policy_cfg.pretrained_path:
            raise RuntimeError("online Actor policy was not initialized from policy.pretrained_path")
        if control.dataset_meta is None:
            raise RuntimeError("online Actor requires dataset metadata")
        if not hasattr(self.policy, "actor") or not hasattr(self.policy, "extract_rlt_features"):
            raise RuntimeError(f"policy.type={self.policy_cfg.type} has no online Actor interface")
    ...
    def _load_actor_head(self) -> None:
        if self._actor_checkpoint is None:
            raise FileNotFoundError(f"No Actor weights found at {self.cfg.actor_checkpoint_path}")
        ...
        if self._actor_checkpoint.name == "actor_critic.pt":
            ...
            if source_base and Path(source_base).expanduser().resolve() != self.policy_path.resolve():
                raise ValueError(f"Actor checkpoint base {source_base} does not match VLA {self.policy_path}")
        ...
        logging.info("[ACTOR] Loaded online Actor head from %s", self._actor_checkpoint)
    ...
    def reload_if_changed(self) -> None:
        ...
        if checkpoint is None:
            ...
            logging.error("[ACTOR] Actor checkpoint disappeared; falling back to VLA-only.")
            return
...
def actor_cli(cfg: ActorPipelineConfig):
    ...
    logging.info("[ACTOR] mode=%s save_format=%s", cfg.actor_mode, cfg.save_format)
    ...
    if cfg.actor_only.enabled:
        run_actor_only(cfg=cfg, shutdown_event=shutdown_event, actor_control=actor_control)
    else:
        run_actor_online(cfg=cfg, shutdown_event=shutdown_event, actor_control=actor_control)
...
def run_actor_online(
    ...
    if cfg.actor_vla_policy.enabled:
        logging.info(
            "[ACTOR] online Actor actions use actor_checkpoint_path; "
            "received learner parameters are ignored by this runtime."
        )
...
def act_with_policy(
    ...
    display_data = cfg.env.processor.observation is not None and cfg.env.processor.observation.display_cameras
    if display_data:
    ...
    policy_action_smoother = _OnlinePolicyActionSmoother(action_names, window=9) if action_names else None
    ...
    robot_action_sender = _RetryingActionSender(robot, "robot.send_action") if robot is not None else None
    ...
    if actor_control is not None and actor_control.cfg.actor_mode == "online_actor":
        policy = make_policy(cfg=cfg.policy, ds_meta=actor_control.dataset_meta, env_cfg=None)
        for name in ("critic_ensemble", "critic_target"):
            critic = getattr(policy, name, None)
            if critic is not None:
                critic.to("cpu")
        actor_control.policy = policy
    else:
    ...
    vla_runtime = (
        OnlineActorRuntime(actor_control)
        if actor_control is not None and actor_control.cfg.actor_mode == "online_actor"
        else ActorVLARuntime(cfg)
        if cfg.actor_vla_policy.enabled
        else None
    )
    ...
    if actor_control is not None:
        actor_control.smoother = policy_action_smoother
    keyboard_controller = (
        ActorKeyboardController(actor_control)
        if actor_control is not None
        and (actor_control.task_hotkeys is not None or actor_control.cfg.actor_mode == "online_actor")
        ...
        *,
        ...
        if should_send and len(list_transition_to_send_to_learner) > 0:
            ...
            needs_compact = cfg.online_transition.enabled or (
                actor_episode_writer is not None and actor_episode_writer.save_format == "transition"
            )
            if needs_compact:
                if vla_runtime is None:
    ...
    try:
        for interaction_step in range(cfg.policy.online_steps):
            ...
            if pending_step is not None:
                ...
                complementary_info["success"] = torch.tensor([float(episode_outcome == EPISODE_SUCCESS)])
                complementary_info["failure"] = torch.tensor([float(episode_outcome == EPISODE_FAILURE)])
            ...
            if teleop_device is not None:
                teleop_action = _run_with_connection_retry("teleop.get_action", teleop_device.get_action)
                act_processed_teleop = teleop_action_processor((teleop_action, raw_observation))
