...
class PiperLeader(Teleoperator):
    ...
    def _send_command_mode(self) -> None:
        ...
        # logging.info(f"leader speed: {self.config.command_speed_ratio}")
        self.arm.MotionCtrl_2(0x01, 0x01, self.config.command_speed_ratio, mit_mode)
        # self.arm.MotionCtrl_2(0x01, 0x01, 5, mit_mode)
        self._last_mode_refresh_t = time.monotonic()

    def _seed_command_target_to_current_pose(self) -> None:
        """用当前实测关节角覆盖固件的 JointCtrl 位置目标寄存器，实现“零位移”交接。

        重力补偿（反驱）期间只发 JointMitCtrl，位置目标寄存器仍是接管前最后一次 JointCtrl 的值；
        直接切回位置跟随模式会让主臂冲回那个旧目标。先读当前 6 关节反馈、原样下发一条 JointCtrl，
        让目标=当前姿态，从而停在人工介入后的最新位置，等待后续 send_feedback 接管。
        """
        joint_msg = self.arm.GetArmJointMsgs()
        joint_state = getattr(joint_msg, "joint_state", None)
        if joint_state is None:
            return
        # joint_state.joint_i 的 raw 单位（毫度）与 JointCtrl 入参一致，无需换算。
        current = [int(getattr(joint_state, joint_name, 0)) for joint_name in PIPER_JOINT_NAMES]
        self.arm.JointCtrl(*current)
    ...
    def set_manual_control(self, enabled: bool) -> None:
        ...
        if not enabled and self._manual_control_enabled is not False:
            ...
            if self.config.sync_gripper:
                # 3、给电机上力矩，解除电机使能，回到可拖动状态。此时主臂可以被人工直接拖动，且不受重力补偿线程的MIT指令干扰。
    ...
    def get_teleop_events(self) -> dict[str, Any]:
        """Return HIL event flags expected by the online RL processor.

        Piper leader provides continuous joint targets but has no buttons for
