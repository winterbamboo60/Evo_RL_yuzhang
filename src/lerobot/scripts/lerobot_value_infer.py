...
from lerobot.scripts.value_infer_viz import (
    ...
    export_advantage_overlay_videos,
    export_indicator_overlay_videos,
)
...
def run_value_inference_pipeline(
    ...
    if not cfg.acp.enable:
        ...
        if accelerator.is_main_process:
            ...
            if cfg.viz.enable:
                if cfg.viz.indicator_only:
                    # 仅可视化二值化优势/劣势：直接读取数据集中已有的 indicator 列。
                    if cfg.acp.indicator_field not in raw_frames.column_names:
                        raise KeyError(
                            f"Missing indicator field '{cfg.acp.indicator_field}' in dataset while "
                            "'viz.indicator_only=true' and 'acp.enable=false'."
                        )
                    ind_viz_paths = export_indicator_overlay_videos(
                        dataset=dataset,
                        indicator_field=cfg.acp.indicator_field,
                        advantage_field=cfg.acp.advantage_field,
                        viz_episodes=cfg.viz.episodes,
                        video_key=cfg.viz.video_key,
                        video_keys=cfg.viz.video_keys,
                        output_dir=output_dir / "indicator_viz",
                        overwrite=cfg.viz.overwrite,
                        vcodec=cfg.viz.vcodec,
                        frame_storage_mode=cfg.viz.frame_storage_mode,
                        smooth_window=cfg.viz.smooth_window,
                    )
                    viz_outputs = [str(p) for p in ind_viz_paths]
                    logging.info(
                        "Exported %d advantage/disadvantage indicator overlay videos to %s/indicator_viz",
                        len(ind_viz_paths),
                        output_dir,
                    )
                else:
                    viz_outputs = _export_visualization_outputs(dataset=dataset, cfg=cfg, output_dir=output_dir)
                    # 非 indicator_only 模式下，除价值/优势曲线外，额外导出最终的二值化
                    # 优势/劣势结论视频（读取数据集中已有的 indicator 列）。
                    if cfg.acp.indicator_field not in raw_frames.column_names:
                        raise KeyError(
                            f"Missing indicator field '{cfg.acp.indicator_field}' in dataset while "
                            "'viz.indicator_only=false' and 'acp.enable=false'."
                        )
                    ind_viz_paths = export_indicator_overlay_videos(
                        dataset=dataset,
                        indicator_field=cfg.acp.indicator_field,
                    ...
                    viz_outputs = viz_outputs + [str(p) for p in ind_viz_paths]
                    logging.info(
                        "Exported %d advantage/disadvantage indicator overlay videos to %s/indicator_viz",
                        len(ind_viz_paths),
                        output_dir,
    ...
    # 解析并预训练模型目录，加载价值函数（Pistar06）和前处理器
    ...
    if accelerator.is_main_process:
        ...
        if cfg.acp.enable:
            ...
            # 密集奖励，r_t = V_t - V_{t+1}（最后一帧 = V_T）
            rewards = _compute_dense_rewards_from_targets(value_targets, episode_indices, frame_indices)
            # N步优势计算，A_t = （Σr + V_t+n） - V_t
            # （Σr + V_t+n）代表从当前状态出发，到最终状态实际的价值。此处使用N步估计，用价值模型替换掉 N 步之后的部分，减少了因为长期的积分导致的噪声大
            # 而V(o_{t+N}) 是价值模型对大量轨迹的平均估计 → 它已经在训练时平滑掉了单条轨迹的随机性 → 方差远小于直接累加真实回报
            ...
            # 阈值计算，根据优势的分布为每个任务计算一个阈值，使得优势高于该阈值的比例约等于cfg.acp.positive_ratio
            thresholds = _compute_task_thresholds(
                task_indices=task_indices,
                advantages=advantages,
                positive_ratio=cfg.acp.positive_ratio,
            )
            # 二值化，优势≥阈值 → 1，否则 → 0；干预帧强制为 1
            ...
            # 统计正样本比例以供分析，该字段标记"高优势"帧，通常用于后续策略训练中的正样本筛选
        ...
        # 将 value、advantage、indicator 三列直接写入 Parquet 文件（原地修改）
        ...
        viz_outputs: list[str] = []
        if cfg.viz.enable:
            # indicator_only：仅导出二值化优势/劣势结论视频，跳过价值曲线与软优势曲线。
            if not cfg.viz.indicator_only:
                # 用本次推理刚算出的真实数组作图，而非读取数据集列（多个 ACP 字段同名时列会互相覆盖）。
                viz_outputs = _export_visualization_outputs(
                    dataset=dataset,
                    cfg=cfg,
                    output_dir=output_dir,
                    values_override=predicted_values,
                    advantages_override=advantages,
                    indicators_override=indicators,
                    targets_override=value_targets,
                )
                if advantages is not None:
                    ad_viz_paths = export_advantage_overlay_videos(
                        dataset=dataset,
                        raw_advantages_all=advantages,
                        n_step=cfg.acp.n_step,
                        viz_episodes=cfg.viz.episodes,
                        video_key=cfg.viz.video_key,
                        video_keys=cfg.viz.video_keys,
                        output_dir=output_dir / "ad_viz",
                        overwrite=cfg.viz.overwrite,
                        vcodec=cfg.viz.vcodec,
                        frame_storage_mode=cfg.viz.frame_storage_mode,
                        smooth_window=cfg.viz.smooth_window,
                    )
                    logging.info("Exported %d advantage overlay videos to %s/ad_viz", len(ad_viz_paths), output_dir)

            if indicators is not None:
                # 将二值化的优势/劣势结论（indicator: 1=优势, 0=劣势）逐帧叠加到视频上，
                # 直观展示每一帧实际上被判定为优势还是劣势。
                ind_viz_paths = export_indicator_overlay_videos(
                    dataset=dataset,
                    raw_indicators_all=indicators,
                    raw_advantages_all=advantages,
                    viz_episodes=cfg.viz.episodes,
                    video_key=cfg.viz.video_key,
                    video_keys=cfg.viz.video_keys,
