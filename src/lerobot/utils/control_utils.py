...
from lerobot.utils.recording_events import EventConfig, event_marker_key
...
class TTYKeyboardListener:
    ...
    def __init__(
        ...
        event_key_bindings: dict[str, list[str]] | None = None,
        ...
        # Key that resets the arms to the initial pose and re-records the current episode.
        self.reset_episode_key = reset_episode_key.lower() if reset_episode_key else None
        # {key_char: [event_names]} for per-frame quality events (bad_depth/bad_edge).
        self.event_key_bindings = event_key_bindings or {}
    ...
    def _handle_key(self, key: str):
        ...
        if normalized == "RIGHT":
            ...
            self.events["exit_early"] = True
        elif self.reset_episode_key and normalized == self.reset_episode_key:
            print(
                f"'{self.reset_episode_key}' key pressed. Resetting arms to initial pose and re-recording episode..."
            )
            self.events["reset_episode"] = True
            self.events["rerecord_episode"] = True
            self.events["exit_early"] = True
        elif normalized in self.event_key_bindings:
            # Momentary marker: flag the current frame as a quality event without ending the episode.
            event_names = self.event_key_bindings[normalized]
            for event_name in event_names:
                self.events[event_marker_key(event_name)] = True
            print(f"'{normalized}' key pressed. Marking current frame as {event_names}...")
...
def init_keyboard_listener(
    ...
    # Set when the reset key is pressed: re-record the current episode after homing the arms
    # back to the stored initial pose.
    events["reset_episode"] = False
    # Set by record_loop when the episode ended by reaching the max recording time (not a key press).
    events["episode_timeout"] = False
    # Momentary markers for per-frame quality events; consumed and reset by record_loop.
    # The set of events and their hotkeys is driven by the (optional) event config.
    if event_config is not None:
        for event_name in event_config.events:
            events[event_marker_key(event_name)] = False
    # {key_char: [event_names]} resolved from the event config file.
    event_key_bindings = dict(event_config.key_bindings) if event_config is not None else {}
        ...
        def on_press(key):
            try:
                if key == keyboard.Key.right:
                    ...
                    and hasattr(key, "char")
                    and key.char
                    and key.char.lower() == reset_episode_key.lower()
                ):
                    print(
                        f"'{reset_episode_key}' key pressed. Resetting arms to initial pose and re-recording episode..."
                    )
                    ...
                    and hasattr(key, "char")
                    and key.char
                    and key.char.lower() in event_key_bindings
                ):
                    # Momentary marker: flag the current frame as a quality event without ending the episode.
                    event_names = event_key_bindings[key.char.lower()]
                    for event_name in event_names:
                        events[event_marker_key(event_name)] = True
                    print(f"'{key.char.lower()}' key pressed. Marking current frame as {event_names}...")
            except Exception as e:
    ...
    if sys.stdin.isatty():
        listener = TTYKeyboardListener(
            ...
            reset_episode_key=reset_episode_key,
        )
        listener.start()
        logging.warning(
