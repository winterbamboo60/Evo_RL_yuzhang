...
```bash
python -m lerobot.onlineRL_evoRL.learner --config_path src/lerobot/configs/train_config_hilserl_so100.json
```
...
from lerobot.onlineRL_evoRL.wandb_utils import WandBLogger
...
def train(cfg: TrainRLServerPipelineConfig, job_name: str | None = None):
    ...
    # Setup logging
    from lerobot.onlineRL_evoRL.wandb_utils import make_logger
...
def process_transitions(
    ...
    while not transition_queue.empty() and not shutdown_event.is_set():
        ...
        for transition in transition_list:
            ...
            # Add intervention transitions to the offline buffer when available.
            complementary_info = transition.get("complementary_info") or {}
            is_intervention = complementary_info.get("is_intervention", 0.0)
            if isinstance(is_intervention, torch.Tensor):
                is_intervention = bool(is_intervention.detach().float().max().item() > 0.5)
            else:
