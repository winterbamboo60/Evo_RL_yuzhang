...
class PiperFollower(Robot):
    ...
    def _send_motion_mode(self) -> None:
        ...
        # logging.info(f"follower speed: {self.config.speed_ratio}")
    ...
    def send_action(self, action: RobotAction, add_offset: bool = False) -> RobotAction:
        ...
        if self.config.sync_gripper and "gripper.pos" in action:
            ...
            # 夹爪非线性衰减已上移到 recording_loop._attenuate_gripper_value（与 inference 一致），
            # 此处不再二次衰减，只做范围安全 clip 后直接下发。
            gripper_pos_raw = int(np.clip(unit_to_milli(gripper_target), 0, 101100))
