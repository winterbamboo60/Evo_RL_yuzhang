...
def _resize_sequence_parameter(parameter: torch.Tensor, seq_len: int) -> torch.Tensor:
    """Resize a learned sequence table without imposing a fixed PI05 prefix length."""
    if parameter.shape[0] == seq_len:
        return parameter
...
class RLTTokenEncoder(nn.Module):
    ...
    def forward(
        ...
        prefix_pos = _resize_sequence_parameter(self.prefix_pos_enc, seq_len).to(
...
class RLTTokenDecoder(nn.Module):
    ...
    def forward(self, rl_tokens: torch.Tensor, target_seq_len: int | None = None):
        ...
        prefix_tokens = (
            _resize_sequence_parameter(self.prefix_token_embed, target_seq_len)
            .to(device=rl_tokens.device, dtype=rl_tokens.dtype)
            .unsqueeze(0)
            .expand(batch_size, -1, -1)
        )
        prefix_pos = _resize_sequence_parameter(self.prefix_pos_enc, target_seq_len).to(
            device=rl_tokens.device, dtype=rl_tokens.dtype
        )
...
class RLTTokenTransformer(nn.Module):
    ...
    @staticmethod
    def _init_multihead_attention(module: nn.MultiheadAttention) -> None:
    ...
    @staticmethod
    def _init_layer_norm(module: nn.LayerNorm) -> None:
    ...
    def reset_parameters(self) -> None:
        for module in [*self.encoder.modules(), *self.decoder.modules()]:
            if isinstance(module, nn.Linear):
                ...
                self._init_multihead_attention(module)
            elif isinstance(module, nn.LayerNorm):
                self._init_layer_norm(module)

        with torch.no_grad():
            self.encoder.rl_token_embed.copy_(
                sinusoidal_pe_init(self.num_rl_tokens, self.embed_dim)
            )
            self.encoder.prefix_pos_enc.copy_(
                sinusoidal_pe_init(self.prefix_seq_len, self.embed_dim)
            )
            self.encoder.rl_token_pos_enc.copy_(
                sinusoidal_pe_init(self.num_rl_tokens, self.embed_dim)
            )
            self.decoder.prefix_token_embed.copy_(
                sinusoidal_pe_init(self.prefix_seq_len, self.embed_dim)
            )
            self.decoder.prefix_pos_enc.copy_(
                sinusoidal_pe_init(self.prefix_seq_len, self.embed_dim)
            )
            self.decoder.rl_token_pos_enc.copy_(
                sinusoidal_pe_init(self.num_rl_tokens, self.embed_dim)
            )
    ...
    def loss(
        ...
        if mask is not None:
            ...
            per_sample_denom = torch.clamp(
                mask_expanded.sum(dim=(1, 2)) * prefix_embs.shape[-1], min=1.0
            )
            per_sample_mse = sq_error.sum(dim=(1, 2)) / per_sample_denom
            denom = torch.clamp(mask_expanded.sum() * prefix_embs.shape[-1], min=1.0)
            mse = sq_error.sum() / denom
        else:
            per_sample_mse = sq_error.mean(dim=(1, 2))
            mse = per_sample_mse.mean()

        loss = per_sample_mse if reduction == "none" else mse
        return loss, {
            "mse": mse,
    ...
    def forward(
        ...
        return self.loss(prefix_embs, mask, reduction=reduction)
