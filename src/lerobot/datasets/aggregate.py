...
def validate_all_metadata(all_metadata: list[LeRobotDatasetMetadata]):
    """Validate merge compatibility and return a union feature schema.

    Non-visual features may be missing from some datasets; aggregate_data fills
    those columns with dtype-correct empty values. Image/video features must be
    present in every dataset because their data cannot be synthesized.
    """

    fps = all_metadata[0].fps
    robot_type = all_metadata[0].robot_type
    features = dict(all_metadata[0].features)
    ...
    for meta in tqdm.tqdm(all_metadata, desc="Validate all meta data"):
        ...
        for key, feature in meta.features.items():
            ...
            if current["dtype"] != feature["dtype"] or tuple(current["shape"]) != tuple(feature["shape"]):
                raise ValueError(
                    f"Feature '{key}' has incompatible schema: "
                    f"{current['dtype']}{tuple(current['shape'])} vs "
                    f"{feature['dtype']}{tuple(feature['shape'])}."
                )

    all_keys = set(features)
    for meta in all_metadata:
        missing = all_keys - set(meta.features)
        missing_visual = [
            key for key in missing if features[key]["dtype"] in ("image", "video")
        ]
        if missing_visual:
            raise ValueError(
                f"Dataset {meta.repo_id} is missing visual features {missing_visual}; "
                "image/video columns cannot be filled automatically."
            )
        if missing:
            logging.info(
                "Dataset %s is missing features %s; filling with empty defaults.",
                meta.repo_id,
                sorted(missing),
            )
...
def _feature_default_value(feature: dict):
    dtype = feature["dtype"]
    ...
    if shape == (1,):
        return np_dtype(0).item() if hasattr(np_dtype(0), "item") else np_dtype(0)
    return np.zeros(shape, dtype=np_dtype)


def _fill_missing_data_columns(df: pd.DataFrame, src_meta: LeRobotDatasetMetadata, dst_meta: LeRobotDatasetMetadata):
    missing = [
        key
        for key, feature in dst_meta.features.items()
        if key not in src_meta.features and feature["dtype"] not in ("image", "video")
    ]
    for key in missing:
        default = _feature_default_value(dst_meta.features[key])
        if isinstance(default, np.ndarray):
            df[key] = [default.copy() for _ in range(len(df))]
        else:
            df[key] = default
    return df


def _zero_feature_stats(feature: dict, count: int) -> dict[str, np.ndarray]:
    shape = tuple(feature["shape"])
    stat_shape = (1,) if shape == (1,) else shape
    zero = np.zeros(stat_shape, dtype=np.float64)
    return {
        "min": zero.copy(),
        "max": zero.copy(),
        "mean": zero.copy(),
...
def _fill_missing_episode_stats(
    df: pd.DataFrame,
    ...
    missing = [
        key
        for key, feature in dst_meta.features.items()
        if key not in src_meta.features and feature["dtype"] not in ("string", "image", "video")
    ]
    for key in missing:
        for stat_key in STAT_KEYS:
            col = f"stats/{key}/{stat_key}"
            if col in df.columns:
                continue
            values = []
            for length in df["length"]:
                stats = _zero_feature_stats(dst_meta.features[key], int(length))
                values.append(stats[stat_key])
            df[col] = values
    return df


def _metadata_stats_with_filled_defaults(meta: LeRobotDatasetMetadata, features: dict) -> dict:
    stats = dict(meta.stats) if meta.stats is not None else {}
    missing = [
        key
        for key, feature in features.items()
        if key not in meta.features and feature["dtype"] not in ("string", "image", "video")
    ]
    for key in missing:
        stats[key] = _zero_feature_stats(features[key], meta.total_frames)
    return stats
...
def aggregate_data(src_meta, dst_meta, data_idx, data_files_size_in_mb, chunk_size):
    ...
    for src_chunk_idx, src_file_idx in unique_chunk_file_ids:
        ...
        df = _fill_missing_data_columns(df, src_meta, dst_meta)
...
def aggregate_metadata(src_meta, dst_meta, meta_idx, data_idx, videos_idx):
    ...
    for chunk_idx, file_idx in chunk_file_ids:
        ...
        df = pd.read_parquet(src_path)
        df = _fill_missing_episode_stats(df, src_meta, dst_meta)
        df = update_meta_data(
            df,
            dst_meta,
            meta_idx,
...
def finalize_aggregation(aggr_meta, all_metadata):
    ...
    aggr_meta.stats = aggregate_stats([_metadata_stats_with_filled_defaults(m, aggr_meta.features) for m in all_metadata])
