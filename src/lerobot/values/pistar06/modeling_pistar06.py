...
class Pistar06Model(nn.Module):
    ...
    def forward(
        ...
        # 图片预处理：调整分辨率、归一化，并根据 attention mask 进行遮挡。然后编码视觉和语言输入，得到特征表示。
        # 接着将视觉特征通过投影层映射到融合维度，并根据 attention mask 进行加权平均池化得到图像的全局表示。
        # 语言特征则通过另一个投影层得到语言的全局表示。
        # 最后将图像和语言的全局表示拼接起来，经过 LayerNorm 和 MLP 头得到最终的价值预测 logits。
        ...
        # 这里输出的image_features不是一个完整的feature，需要检查为什么需要额外取pooler_output
        # image_features = image_features.pooler_output.to(dtype=feature_dtype)
...
class Pistar06Policy(PreTrainedPolicy):
    ...
    def build_training_raw_batch_hook(self, dataset, targets_cfg):
        ...
        # episode_index：查 episode 元数据（length、episode_success）
        # frame_index：帧在 episode 内的相对步数（用来算 remaining_steps）
        # index：全局帧索引（用于 lookup）
        ...
        # 构建全局帧索引到 value target 的 lookup table，训练时根据 batch 中的 index 字段快速查表得到对应的 value target。
        ...
        # 返回一个 hook 函数，在训练时被调用。这个函数会从 batch 中取出全局帧索引，查表得到对应的 value target，并把它添加到 batch 中指定的 target_key 字段里，供模型训练使用。
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        # 将target的[-1,0]投影到其中201个bin中，把回归问题转化为分类问题
        soft_target = project_values_to_bins(value_target, bin_centers)
        log_probs = functional.log_softmax(logits, dim=-1)
        # 计算每个样本的交叉熵损失。这个损失值是基于模型输出的离散概率分布（logits）和目标分布（soft_target）计算出来的。
        ...
        # 计算预测价值与目标价值之间的平均绝对误差（MAE），作为一个辅助指标来评估模型的回归性能。这个指标不参与梯度计算，只用于监控训练过程中的预测质量。
        # 基于机械臂执行过程中每一帧的视觉信息以及状态，由模型预测一个连续的价值估计，并与基于剩余步数和成功/失败信息计算出来的归一化未来价值进行比较，来指导模型学习更准确的价值函数。
