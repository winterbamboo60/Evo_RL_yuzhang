...
from lerobot.policies.pi05_onlineRL.configuration_pi05_online_rl import DEFAULT_IMAGE_SIZE, PI05OnlineRLConfig
from lerobot.policies.pi05_onlineRL.rlt_token_transformer import RLTTokenTransformer
...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05OnlineRLConfig, rtc_processor: RTCProcessor | None = None):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        # images：输入图像，视觉观测
        # tokens：文本/语言指令的token序列
        # actions：真实动作（训练目标），B=batch，T=chunk_size，D=动作维度
        ...
        # 线性插值，得到当前时间步的noisy action（X_t）
        # 这是 Flow Matching 的核心：在真实动作（t=0）和纯噪声（t=1）之间线性插值，训练模型拟合这条"流"的方向向量 u_t
        x_t = time_expanded * noise + (1 - time_expanded) * actions
        u_t = noise - actions

        # 将图像和语言 token 编码为 条件前缀嵌入，作为 Transformer 的上下文
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(images, img_masks, tokens, masks)
        # 将 x_t（加噪动作）和时间步 t 编码为 后缀嵌入，输入给 Expert 分支。adarms_cond 是给 AdaRMS（自适应归一化）用的时间条件。
        ...
        # Prefix 和 Suffix 共同组成完整序列，通过 2D mask 控制哪些 token 能相互 attend（前缀单向注意，后缀可 attend 前缀）。
...
def compute_chunk_td_target(
    ...
    """Discount rewards inside a chunk and bootstrap only complete nonterminal chunks."""
    ...
    bootstrap = (
        valid_action_mask.sum(dim=-1).eq(horizon)
        & ~done.bool()
        & ~truncated.bool()
    )
...
class PI05OnlineRLPolicy(PreTrainedPolicy):
    """PI05 Policy for LeRobot."""

    config_class = PI05OnlineRLConfig
    name = "pi05_online_rl"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.actor = RLTChunkActor(
            config.z_dim, config.proprio_dim, config.chunk_size, self.action_dim,
            config.actor_hidden_dims, config.fixed_std, config.reference_dropout_prob,
        ).to(config.device)
        self.critic_ensemble = RLTChunkCriticEnsemble(
            config.z_dim, config.proprio_dim, config.chunk_size, self.action_dim,
            config.critic_hidden_dims, config.num_critics,
        ).to(config.device)
        self.critic_target = deepcopy(self.critic_ensemble).requires_grad_(False)
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Could not load model.safetensors: {e}") from e
            ...
            rl_prefixes = ("actor.", "critic_ensemble.", "critic_target.")
            rl_missing_keys = [key for key in missing_keys if key.startswith(rl_prefixes)]
            non_rlt_missing_keys = [
                key for key in missing_keys
                if not key.startswith("model.rlt_module.") and not key.startswith(rl_prefixes)
            ]

            if rl_missing_keys:
                print(f"RL heads initialized locally ({len(rl_missing_keys)} keys).")
            ...
            if strict and (non_rlt_missing_keys or (model.config.use_rlt and rlt_missing_keys)):
                raise RuntimeError(
            ...
            raise RuntimeError(f"Could not load PI05OnlineRL checkpoint: {e}") from e
    ...
    def forward_vla(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
    ...
    def extract_rlt_features(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        proprio = batch[OBS_STATE][..., : self.config.proprio_dim]
        return {"z_rl": outputs["z_rl"], "proprio": proprio, "ref_action": outputs["actions"]}

    def forward(self, batch: dict[str, Tensor], model: str = "critic") -> dict[str, Tensor]:
        ...
        if model == "critic":
            ...
            with torch.no_grad():
                ...
                target_q = compute_chunk_td_target(
                    batch["reward"], mask, next_q, batch["done"], batch["truncated"],
                    self.config.discount,
                )
            return {
                "loss_critic": F.mse_loss(q_values, target_q.unsqueeze(0).expand_as(q_values)),
                "q": q_values.mean().detach(),
        ...
        if model == "actor":
            ...
            bc = bc / (mask.sum() * self.action_dim).clamp_min(1)
            return {
                "loss_actor": -self.config.q_weight * actor_q + self.config.bc_weight * bc,
                "bc_loss": bc.detach(),
                "actor_q": actor_q.detach(),
            }

        raise ValueError(f"Unsupported pi05_online_rl model: {model}")
