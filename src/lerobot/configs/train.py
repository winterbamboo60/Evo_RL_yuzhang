...
class TrainPipelineConfig(HubMixin):
    ...
    num_workers: int = 4
    batch_size: int = 8
    # Number of micro-batches whose gradients are accumulated before each optimizer update.
    # Each optimizer step therefore consumes `batch_size * gradient_accumulation_steps` samples
    # per process (i.e. it emulates training with that larger effective batch size).
    ...
    use_8bit_optimizer: bool = False  # Replace AdamW with 8-bit AdamW (bitsandbytes) to cut optimizer state ~75%
    ...
    def validate(self) -> None:
        ...
        if not self.use_policy_training_preset and (self.optimizer is None or self.scheduler is None):
            ...
            preset = self.policy.get_optimizer_preset()
            if self.use_8bit_optimizer:
                from lerobot.optim.optimizers import AdamW8bitConfig
                self.optimizer = AdamW8bitConfig(
                    lr=preset.lr,
                    betas=getattr(preset, "betas", (0.9, 0.999)),
                    eps=getattr(preset, "eps", 1e-8),
                    weight_decay=preset.weight_decay,
                    grad_clip_norm=preset.grad_clip_norm,
                )
            else:
                self.optimizer = preset
