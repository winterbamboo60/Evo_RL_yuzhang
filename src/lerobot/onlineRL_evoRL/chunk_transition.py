"""Sliding-window construction used by EvoRL compact episodes."""
...
def sliding_window_observation_indices(
    episode_length: int, horizon: int = 50, stride: int = 1
) -> list[int]:
    if horizon <= 0:
...
def build_sliding_window_transitions(
    episode: list[dict], horizon: int = 50, stride: int = 1
) -> list[dict]:
    if horizon <= 0:
        raise ValueError("horizon must be positive")
    if stride <= 0:
